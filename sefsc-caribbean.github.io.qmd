---
title: "sefsc-caribbean.github.io"
...
librarian::shelf(readxl, purrr, dplyr, stringr, tidyselect)

sheets <- readxl::excel_sheets("Caribbean_Fisheries_Projects.xlsx")
file <- purrr::map_dfr(
  .x = purrr::set_names(sheets),
  read_excel,
...
sections <- file |>
  dplyr::select(matches(section_var)) |>
  dplyr::distinct() |>
  dplyr::pull()
...
for (i in 1:length(sections)) {
  ...
  sub_section <- file |>
    dplyr::filter(get(section_var) == sections[i]) |>
    dplyr::select(-tidyselect::any_of(section_var))

  for (j in 1:nrow(sub_section)) {
    cat("\n##", sub_section$title[j], "\n")
    cat("\n#### ", stringr::str_to_title(section_var),
      ": ", sections[i],
      sep = ""
    )
    cat("\n#### Contact: [", sub_section$poc[j],
      "](", sub_section$website[j], ")",
      sep = ""
    )
    ...
    record <- sub_section[j, ] |>
      dplyr::select(
        tidyselect::where(~ !any(is.na(.))), -title, -region,
    ...
    for (k in 1:ncol(record)) {
      cat(
        paste0(
          "- ", stringr::str_to_title(names(record)[k]),
          ": ", dplyr::pull(record[k]), "\n"
        )
      )
    }
